...
**LangChain migration status:** functionally done — categorize and NL input both run on `get_llm(...).with_structured_output(...)` (default provider **Groq**, switchable via `LLM_PROVIDER`), and PF-22c wired in LangSmith tracing. The deprecated raw-SDK stack (`app/ai/client.py`, `app/ai/agent.py`, `app/ai/tools.py` — i.e. `call_llm`, `run_agent`, `@tool`/`Tool`/`TOOL_REGISTRY`) still exists but has **no caller**: PF-23 (NL input) was built on `with_structured_output` instead of `run_agent`, so deleting that stack is now standalone cleanup tracked as **PF-22d** (and the chat advisor PF-35/36 will build on LangChain-native `bind_tools`, not `run_agent`). Both stacks coexist until PF-22d. **New AI features use `get_llm(...)` only — never `run_agent`/`@tool`.**
...
- **Categorize pattern:** `app/services/categorize.py::_suggest_with_llm` calls `get_llm(feature="categorize").with_structured_output(_SuggestCategoryOutput).invoke([SystemMessage(...), HumanMessage(...)])`. No `db` is passed to the LLM layer — the audit row is written by the `AuditCallbackHandler` attached inside `get_llm`, which opens its own session. The system message is a **plain string** (`_SYSTEM_PROMPT` + category list, via the now-public `categorize.format_categories(...)`). The rules-first path (`suggest_from_rules` → `_match_rule`, `accept_rule`, etc.) is untouched and never calls the LLM. `categorize.format_categories` is **public and shared** with nl_input; `suggest_from_rules(note, db)` is the **rules-only** entry point (no LLM) that nl_input reuses.
- **NL input pattern (PF-23):** `app/services/nl_input.py::parse_nl_entry(db, nl_input_request)` mirrors categorize — one `get_llm(feature="nl_input").with_structured_output(_NLParse).invoke([SystemMessage(...), HumanMessage(text)])` call in `_extract_fields_with_llm`. The model only **extracts** (kind, raw rupee `amount`, ISO `occurred_on`, `account_name`/`category_name` chosen from closed lists in the prompt — categories rendered with the shared `categorize.format_categories` — and `note`); **all resolution is Python** — `amount_minor = round(amount*100)` (422 if no amount; FE renders paise→₹), date parsed/defaulted to today (today is passed into the prompt; malformed → today), `account_name` matched against active accounts else `default_account_id` (422 if that doesn't exist; FE sends the user's selected account as the fallback). Endpoint `POST /api/ai/nl-input` lives on the existing `ai_router` (not a new router) and **returns a draft only — never inserts** (FE confirm card saves via `POST /api/transactions`). **Category resolution:** exact name match **within the chosen kind**, else **rules-only** fallback via `categorize.suggest_from_rules(note, db)` — *not* the full `categorize.suggest`. Why: the model already saw the same category list and returned no match, so a second LLM call over that same list is redundant cost; the saved **rules**, though, are the user's learned corrections the model never sees, so they're worth checking. A rule's category is kept only if its `kind` matches the draft (else nulled — the save endpoint enforces `category.kind == txn.kind`). `category_source` ∈ `"matched" | "rule" | "none"`.
- **AI test mocking:** patch the feature's `get_llm` (e.g. `app.services.categorize.get_llm`) and drive the chain via `get_llm.return_value.with_structured_output.return_value.invoke` (set `.return_value`/`.side_effect` to the structured-output model; read `.call_args_list[i].args[0]` to inspect the messages). No real LLM/network in tests. `tests/test_imports.py` shares this via its autouse `mock_llm` fixture. **NL input tests** (`tests/test_nl_input.py`) only patch `app.services.nl_input.get_llm` per-test — the category fallback is rules-only (no LLM), so there's nothing else to mock; exercise the fallback by seeding a `categorization_rules` row.
